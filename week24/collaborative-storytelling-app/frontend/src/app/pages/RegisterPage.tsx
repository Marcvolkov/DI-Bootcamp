...
import type { RegisterRequest } from '@storytelling/types';
...
const RegisterPage = () => {
  ...
  useEffect(() => {
    ...
    if (error) {
      dispatch(showErrorNotification('Registration Failed', error));
    }
  }, [error, dispatch]);
  ...
  const validateForm = () => {
    ...
    if (formData.password.length < 6) {
      dispatch(showErrorNotification('Validation Error', 'Password must be at least 6 characters long'));
      return false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      dispatch(showSuccessNotification('Success', 'Account created successfully!'));
      navigate('/');
    } catch {
